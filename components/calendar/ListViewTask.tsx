...
export function ListViewTask({ task, onClick }: ListViewTaskProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-white dark:bg-slate-700 rounded-xl p-4 hover:shadow-md transition-all text-left group"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h4 className="font-medium text-gray-800 dark:text-gray-100 group-hover:text-[#7a6bc0] dark:group-hover:text-purple-400 transition-colors mb-1">
            {task.title}
          </h4>
          {dateRange && (
        ...
        <div className="flex-shrink-0">
          {task.is_completed ? (
            <div className="w-5 h-5 rounded-full bg-[#8b7dd8] dark:bg-purple-500 flex items-center justify-center">
              <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
              </svg>
            </div>
          ) : (
            <div className="w-5 h-5 rounded-full border-2 border-gray-300 dark:border-gray-500 group-hover:border-[#8b7dd8] dark:group-hover:border-purple-400 transition-colors" />
          )}
        </div>
      </div>
    </button>
