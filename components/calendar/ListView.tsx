    ...
    <div className="p-6">
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-64 flex-shrink-0">
          <div className="bg-[#9a81fc]/10 border border-gray-100 rounded-2xl p-4 space-y-4">
            {/* Botón Nueva Tarea */}
            <button
              onClick={onAddTask}
              className="w-full flex items-center justify-center gap-2 bg-white text-[#7a6bc0] px-4 py-3 rounded-xl hover:bg-[#8b7dd8]/5 transition-colors font-medium shadow-sm"
            >
            ...
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8b7dd8]" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search"
                className="w-full pl-10 pr-4 py-2 bg-white rounded-xl border border-[#8b7dd8]/20 focus:outline-none focus:ring-2 focus:ring-[#8b7dd8]/50 text-sm placeholder:text-[#8b7dd8]/50"
              />
            </div>

            {/* Divisor */}
            <div className="border-t border-[#8b7dd8]/20" />

            {/* Filtros por categoría */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Filter className="w-4 h-4 text-[#8b7dd8]" />
                <span className="text-sm font-medium text-[#7a6bc0]">Filtrar</span>
              </div>
                ...
                {categories.map(category => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative">
                        ...
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                          selectedCategories.includes(category.id)
                            ? 'bg-[#8b7dd8] border-[#8b7dd8]'
                            : 'bg-white border-[#8b7dd8]/50 group-hover:border-[#8b7dd8]'
                        }`}>
                          {selectedCategories.includes(category.id) && (
                      ...
                      <span className="text-xs text-[#8b7dd8] bg-white px-2 py-0.5 rounded-full">
                        {categoryTaskCount}
                      </span>
                    </label>
                  );
        ...
        <div className="flex-1 min-h-[calc(100vh-8rem)] flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              ...
              className="text-[#8b7dd8] hover:text-[#7a6bc0] font-medium transition-colors"
            >
              + Nueva Sección
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 auto-rows-min content-start flex-1">
            ...
            {groupedTasks.size === 0 && (
              <div className="w-full text-center py-16">
                <div className="inline-flex items-center justify-center w-16 h-16 bg-[#8b7dd8]/10 rounded-full mb-4">
                  <Search className="w-8 h-8 text-[#8b7dd8]" />
                </div>
                <h3 className="text-lg font-medium text-gray-800 mb-2">
                  {searchQuery ? "No se encontraron tareas" : "No hay tareas"}
                </h3>
                <p className="text-[#7a6bc0]">
                  {searchQuery
